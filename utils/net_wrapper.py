...
class NetworkWrapper(object):
    ...
    def remove_structures_list(self, fraction_to_prune_list, prune_what,
                               prune_how, group_size, ranking_noise):
        """ Physically remove channels and corresponding filters from the model
        
        Returns the compute-sparsity list of the model
        fraction_to_prune_list: (list)

        """
        assert isinstance(fraction_to_prune_list, list)
        assert len(fraction_to_prune_list) == self.num_pruned_layers()
        fraction_after_prune_list = []

        for state_id in range(self.num_pruned_layers()):
            fraction_to_prune_layer = fraction_to_prune_list[state_id]
            layer_id = self.model_metadata.pruned_idxs[state_id]
            apply_thinning = (state_id == self.num_pruned_layers() - 1)
            fraction_after_prune_layer = self.remove_structures(
                layer_id,
                fraction_to_prune=fraction_to_prune_layer,
                prune_what=prune_what,
                prune_how=prune_how,
                group_size=group_size,
                apply_thinning=apply_thinning,
                ranking_noise=ranking_noise)
            fraction_after_prune_list.append(fraction_after_prune_layer)

        return fraction_after_prune_list

    def remove_elements_list(self, fraction_to_prune_list, 
                               prune_how, group_size, ranking_noise):
        """ Physically remove elements from the model
        
        Returns the sparsity list of the model
        fraction_to_prune_list: (list)
        ...
        for state_id in range(self.num_pruned_layers()):
            ...
            weight_name = layer.name + ".weight"
            levels[weight_name] = fraction_to_prune_list[state_id]
        ...
        for state_id in range(self.num_pruned_layers()):
            fraction_to_prune_layer = fraction_to_prune_list[state_id]
            layer_id = self.model_metadata.pruned_idxs[state_id]
            fraction_after_prune_layer = self.remove_elements(
                layer_id,
                fraction_to_prune=fraction_to_prune_layer,
                pruner=pruner)
            fraction_after_prune_list.append(fraction_after_prune_layer)
    ...
    def remove_elements(self, layer_id, fraction_to_prune, 
                          pruner):
        """Physically remove elements from the model

        Returns the sparsity of the layer with index 'layer_id'
        """
        if layer_id not in self.model_metadata.pruned_idxs:
            raise ValueError("idx=%d is not in correct range " % layer_id)
        if fraction_to_prune < 0:
            raise ValueError("fraction_to_prune=%.3f is illegal" %
                             fraction_to_prune)
        if fraction_to_prune == 0:
            return 0
        ...
        msglogger.debug("ADC: trying to remove %.1f%% elements from %s" %
                        (fraction_to_prune * 100,  conv_pname))

        calculate_sparsity = distiller.sparsity
        ...
        if (self.zeros_mask_dict[conv_pname].mask is None
                or 0 == calculate_sparsity(
                    self.zeros_mask_dict[conv_pname].mask)):
            msglogger.debug(
                "remove_structures: aborting because there are no structures to prune"
            )
            return 0
        final_action = calculate_sparsity(
            self.zeros_mask_dict[conv_pname].mask)

        # Use the mask to prune
        self.zeros_mask_dict[conv_pname].apply_mask(conv_p)
        self.model_metadata.reduce_layer_macs(layer, final_action,"element")
        macs_after = self.layer_net_macs(layer)

        # assert 1. - (macs_after / macs_before) == final_action
        if 1. - (macs_after / macs_before) - final_action > 0.001:
            msglogger.info("final_action = %.4f" % final_action)
            msglogger.info("macs_after = %d, macs_before = %d" %
                           (macs_after, macs_before))
            # assert 1. - (macs_after / macs_before) == final_action
        return final_action


    def remove_structures(self, layer_id, fraction_to_prune, prune_what,
                          prune_how, group_size, apply_thinning,
                          ranking_noise):
        """Physically remove channels and corresponding filters from the model
        ...
        msglogger.debug("ADC: trying to remove %.1f%% %s from %s" %
        ...
        if apply_thinning:
            ...
            remove_structures_fn(self.model,
                                 self.zeros_mask_dict,
                                 self.app_args.arch,
                                 self.app_args.dataset,
                                 optimizer=None)
            # distiller.model_summaries.model_summary(self.model, "compute",
            #                                         self.app_args.dataset)
        self.model_metadata.reduce_layer_macs(layer, final_action,prune_what)
        macs_after = self.layer_net_macs(layer)
    ...
    def validate(self, is_episode=True):
        """
        Choose the way to validate the model depend on the number of validation sets.
        If the validation_num = 1: Validate the model on single validation set both per_episode and per_generation.
        If the validation_num > 1: Use the first validation set in per_episode and randomly choose a validation set from 
                                   the rest in per_generation.
        The reason to validate the model in this way is to avoid overfitting in the process of searching for the best schedule
        using Evolutional Algorithm.
        """

        assert len(self.services.validate_fn_list) > 0
        if is_episode:
            top1, top5, vloss = self.services.validate_fn_list[0](
                model=self.model)
            msglogger.info("Validate_fn is using the [0] validation set!\n")
        else:
            from random import randrange
            random_index = -1 if len(self.services.validate_fn_list)==1 else randrange(len(self.services.validate_fn_list) - 1)
            random_fn = self.services.validate_fn_list[random_index + 1]
            top1, top5, vloss = random_fn(model=self.model)
            msglogger.info("Validate_fn is using the [%d] validation set!\n",
                           random_index + 1)
        return top1, top5, vloss

    def test(self):
        top1, top5, vloss = self.services.test_fn(model=self.model)
        distiller.model_summaries.model_summary(self.model, "compute",
                                                self.app_args.dataset)
        return top1, top5, vloss
...
class NetworkMetadata(object):
    def __init__(self, model, dataset, dependency_type, modules_list):
        details = get_network_details(model, dataset, dependency_type,
                                      modules_list)
    ...
    def reduce_layer_macs(self, layer, reduction, prune_what):
        ...
        if not prune_what == "element":
            for dependent_mod in layer.dependencies:
                macs_reduced = self.name2layer(dependent_mod).macs * reduction
                nnz_reduced = self.name2layer(
                    dependent_mod).weights_vol * reduction
    ...
    def name2layer(self, name):
        layers = [
            layer for layer in self.all_layers.values() if layer.name == name
        ]
        if len(layers) == 1:
            return layers[0]
...
def get_network_details(model, dataset, dependency_type, layers_to_prune=None):
    ...
    layers = OrderedDict({
        ...
        for mod_name in unfiltered_layers
        if mod_name in mods and isinstance(mods[mod_name], (torch.nn.Conv2d,
                                                            torch.nn.Linear))
    })
    ...
    for layer_id, (name, m) in enumerate(layers.items()):
        if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Linear):
            ...
            if layers_to_prune is None or name in layers_to_prune:
                ...
                find_dependencies(dependency_type, g, all_layers, name,
                                  new_layer.dependencies)
    ...
    def convert_layer_names_to_indices(layer_names):
        ...
        """
        layer_indices = [
            index for name in layer_names
            for index, layer in all_layers.items() if layer.name == name[0]
        ]
