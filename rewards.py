...
def reward_factory(reward_name):
    ...
    return {
        "mac-constrained":
        (amc_mac_constrained_reward_fn, mac_constrained_clamp_action),
        "accuracy-guaranteed": (amc_accuracy_guarantee_reward_fn, None),
        "mac-constrained-experimental":
        (mac_constrained_experimental_reward_fn, None),
...
def mac_constrained_clamp_action(env, pruning_action):
    ...
    target_reduction = (1. -
                        env.amc_cfg.target_density) * env.original_model_macs
    ...
    msglogger.debug("\t\tflops=%.3f  reduced=%.3f  rest=%.3f  duty=%.3f" %
                    (layer_macs, reduced, rest, duty))
    msglogger.debug("\t\tpruning_action=%.3f  pruning_action_final=%.3f" %
                    (pruning_action, pruning_action_final))
    msglogger.debug(
        "\t\ttarget={:.2f} reduced={:.2f} rest={:.2f} duty={:.2f} flops={:.2f}\n"
        .format(1 - env.amc_cfg.target_density,
                reduced / env.original_model_macs,
                rest / env.original_model_macs, duty / env.original_model_macs,
                layer_macs / env.original_model_macs))
    if pruning_action_final != pruning_action:
        msglogger.debug(
            "pruning_action={:.2f}==>pruning_action_final={:.2f}".format(
                pruning_action, pruning_action_final))
