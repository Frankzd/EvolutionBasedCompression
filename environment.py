...
class Environment(gym.Env):
  ...
  def _collect_fm_reconstruction_samples(self, modules_list):
    ...
    def acceptance_criterion(m, mod_names):
      ...
      return isinstance(
          m,
          (torch.nn.Conv2d, torch.nn.Linear)) and m.distiller_name in mod_names
  ...
  @property
  def paramNumDistribution(self):
    """
            Return the param-num per layer 
        """
    per_layer_params = []
    ...
    for i in range(layer_num):
      ...
      per_layer_params.append(self.net_wrapper.layer_params(layer))
  ...
  def record_best_model(self,
    ...
    if is_val_1:
      ...
      msglogger.info("Record the best generation with the best reward %.3f" %
                     best_reward)
  ...
  def fast_eval(self, pruning_action_list):
    ...
    pruning_action_list = np.clip(pruning_action_list, self.action_low,
                                  self.action_high)
    msglogger.debug(
        "\tAgent clipped pruning_action_list={}".format(pruning_action_list))
    ...
    for state_id in range(len(pruning_action_list)):
      ...
      msglogger.debug("\tbaseline: %.3f" % (1 - pruning_action_list[state_id]))
      # assert math.isclose(
      #     layer_macs_after_action / layer_macs_before_list[state_id],
    ...
    msglogger.info("Generation %d Episode %d is ending" %
                   (self.generation, self.episode))
    reward, top1, top5, vloss = self.compute_reward(total_macs_after_act,
                                                    total_nnz_after_act)
    self.finalize_episode(reward, (top1, top5, vloss), total_macs_after_act,
                          total_nnz_after_act, self.action_history)
  ...
  def step(self, pruning_action_list):
    ...
    # Calculate the final compression rate.
    total_macs_before, _ = self.net_wrapper.get_resources_requirements()
    msglogger.info("\033[1;31m Total_macs_before=%d !\033[0m" %
                   total_macs_before)
    layer_macs_before_list = []
    for state_id in range(len(pruning_action_list)):
      layer_id = self.net_wrapper.model_metadata.pruned_idxs[state_id]
  ...
  def next_generation(self):
    ...
    reward_validation_set_1 = self.ea_cfg.reward_fn(self, top1, top5, vloss,
    ...
    reward_validation_set_2 = self.ea_cfg.reward_fn(self, top1, top5, vloss,
                                                    self.best_total_macs_val_1)
  ...
  def run_test(self):
    ...
    msglogger.info("The final reward on test set is %.3f" % reward_test_set)
  ...
  def finalize_episode(self,
    ...
    if reward > self.best_val_1:
      ...
      msglogger.info(
          "Best reward={} generation={} episode={} top1={} top5={}".format(
              reward, self.generation, self.episode, top1, top5))

    import json
    performance = self.net_wrapper.performance_summary()
    fields = [
        self.episode, top1, reward, total_macs, normalized_macs, normalized_nnz,
  ...
  def finalize_generation(self,
    ...
    if reward > self.best_val_2:
      ...
      msglogger.info("\033[1;31m New best result on Validation set 2 !\033[0m")
      msglogger.info("\033[1;31m Generation: %d Reward: %.3f\033[0m" %
                     (self.generation, reward))
      msglogger.info("Best reward={} generation={} top1={} top5={}".format(
    ...
    self.stats_logger.add_record(fields)
    msglogger.info("Top1:%.2f - compute: %.2f%% - params:%.2f%% - actions:%s",
                   top1, normalized_macs, normalized_nnz, action_history)
    if log_stats:
  ...
  def compute_reward(self, total_macs, total_nnz):
    ...
    accuracies = self.net_wrapper.train(self.ea_cfg.num_ft_epochs, self.episode)
